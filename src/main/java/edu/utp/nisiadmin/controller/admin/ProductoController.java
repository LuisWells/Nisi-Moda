...
public class ProductoController {
    ...
    @GetMapping
    public String listaProductos(Model model, @RequestParam("page") Optional<Integer> page,
                                 @RequestParam("size") Optional<Integer> size) {
        int currentPage = page.orElse(1);
        int pageSize = size.orElse(5);
        model.addAttribute("productos", servicio.obtenerTodosLosProductos(currentPage, pageSize));
